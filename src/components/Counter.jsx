...
const Counter = () => {
  ...
  const { actionEndStart, timePerInterval, intervalGracePeriod } = useContext(MainContext);
  
  return (
	<div className="main-c-counter">
    <Sequence from={0} durationInFrames={intervalGracePeriod * fps} layout="none">
      <p className="main-c-counter__count">0</p>
    </Sequence>
    {
      actionEndStart.map((startFrame, idx) => (
        <Sequence from={startFrame} durationInFrames={(timePerInterval + intervalGracePeriod) * fps} layout="none">
		      <p className="main-c-counter__count">{idx + 1}</p>
        </Sequence>
      ))
    }
